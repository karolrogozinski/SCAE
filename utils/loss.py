...
def get_losses(latent_sample_w, latent_dist_w, beta,
    ...
    if idx_kl <= 100000:
        loss = 1000000*rec_loss + pairwise_tc_loss + 1000000*rec_loss_prop_all\
            + groupwise_tc_loss + 100000*l1norm + sinkhorn_z + sinkhorn_w
    else:
        if w_kl < 100000:
            w_kl += 1
        loss = lambdas[0]*rec_loss + lambdas[1]*pairwise_tc_loss +\
            lambdas[2]*rec_loss_prop_all + lambdas[3]*groupwise_tc_loss +\
            lambdas[4] * w_kl * kl_loss + lambdas[5]*sinkhorn_z +\
            lambdas[6]*sinkhorn_w
